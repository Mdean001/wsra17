			...
			if(preg_match($patternEposta, $trimEposta)==1){
				...
				if(preg_match($patternZailtasuna, $trimZailtasun)==1){
					...
					if(preg_match($patternGaldera, $trimGaldera)==1){
						...
						$sql = "INSERT INTO questions(Eposta, Galdera, ErantzunZuzena, ErantzunOkerra1, ErantzunOkerra2, ErantzunOkerra3, Zailtasuna, GaiArloa) VALUES ('$trimEposta','$trimGaldera','$trimEranZuzen','$trimEranOker1','$trimEranOker2','$trimEranOker3','$trimZailtasun','$trimGaia')";
						
						$egokiSartuta = $connection->query($sql);
						
						if ($egokiSartuta === TRUE) {
							echo "OK: Datu-basean galdera zuzen txertatu da <br/>";
						} else {
